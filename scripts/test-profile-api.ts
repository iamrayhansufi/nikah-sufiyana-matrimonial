...
async function testProfileAPI() {
  ...
  try {    // First, let's check if we have any users in Redis
    ...
    const testUserId = String(allUserIds[0]); // Convert to string
    console.log(`\n2. Testing with user ID: ${testUserId}`);
    
    // Test getting user by ID
    console.log('\n3. Testing database.users.getById...');
    const userById = await database.users.getById(testUserId);
    if (userById) {
